...
class SkillMetadata:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> SkillMetadata:
        """从字典创建"""
        requires_data = data.get("metadata", {}).get("OpenRoboBrain", {}).get("requires", {})
        # 兼容 moltbot 格式
...
class SkillParser(LoggerMixin):
    ...
    def _determine_source(self, path: Path) -> SkillSource:
        ...
        if path_str.startswith(os.path.join(home, ".OpenRoboBrain")):
...
class SkillLoader(LoggerMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            local_dir: 本地技能目录 (默认 ~/.OpenRoboBrain/skills)
            bundled_dir: 内置技能目录
        """
        self._workspace_dir = workspace_dir
        self._local_dir = local_dir or Path.home() / ".OpenRoboBrain" / "skills"
