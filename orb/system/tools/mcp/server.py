...
将OpenRoboBrain能力暴露为MCP Server，供外部系统调用。
"""
...
if TYPE_CHECKING:
    from orb.core import orb

logger = get_logger(__name__)


class ORBMCPServer(LoggerMixin):
    """
    OpenRoboBrain MCP Server
    
    将OpenRoboBrain的技能和资源暴露为MCP Server，
    使外部系统（如Cursor、Claude等）可以调用。
    """
    
    def __init__(
        self,
        OpenRoboBrain: Optional["OpenRoboBrain"] = None,
        ...
        Args:
            OpenRoboBrain: OpenRoboBrain实例（可选）
            name: Server名称
            version: Server版本
        """
        self.OpenRoboBrain = OpenRoboBrain
        self.name = name
    ...
    def _register_default_tools(self) -> None:
        """注册默认工具"""
        # 如果有OpenRoboBrain实例，注册相关工具
        if self.OpenRoboBrain:
            self.register_tool(
                name="execute_skill",
                description="执行OpenRoboBrain技能",
                parameters={
                    "type": "object",
                    "properties": {
                        "skill_name": {
    ...
    async def _handle_execute_skill(
        ...
        """处理技能执行请求"""
        if not self.OpenRoboBrain:
            return {"error": "OpenRoboBrain not initialized"}
    ...
    async def _handle_get_status(self) -> dict:
        ...
        return {
            "running": self.OpenRoboBrain._running if hasattr(self.OpenRoboBrain, "_running") else False,
            "name": "OpenRoboBrain",
