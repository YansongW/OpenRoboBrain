...
from kaibrain.system.services.logger import trace_context, Layer

if TYPE_CHECKING:
    from kaibrain.agent.orchestrator.orchestrator import OrchestratorAgent
    from kaibrain.agent.atomic.cognitive.reasoning import ReasoningAgent
...
class GeneralBehavior(Behavior):
    ...
    async def _direct_llm_call(
        ...
        # 构建系统提示词
        system_prompt = """你是KaiBrain机器人大脑系统。
...
"""
    ...
    def _rule_based_response(self, user_input: str) -> Dict[str, Any]:
        ...
        if any(kw in input_lower for kw in ["你好", "hi", "hello", "嗨"]):
            ...
            result["chat_response"] = "你好！我是KaiBrain机器人，有什么可以帮您的吗？"
